...
var require_library = __commonJS({
  "node_modules/.prisma/saas-client/runtime/library.js"(exports, module) {
    "use strict";
...
// node_modules/.prisma/saas-client/index.js
var require_saas_client = __commonJS({
  "node_modules/.prisma/saas-client/index.js"(exports2) {
    ...
    exports2.Prisma.OrganizationNotificationScalarFieldEnum = {
      id: "id",
      organizationId: "organizationId",
      message: "message",
      type: "type",
    ...
    exports2.Prisma.UserStoreScalarFieldEnum = {
      userId: "userId",
      storeId: "storeId"
    };
    ...
    exports2.Prisma.ProductScalarFieldEnum = {
      ...
      tracksCylinder: "tracksCylinder",
      cylinderSize: "cylinderSize",
      depositAmount: "depositAmount",
      emptyStock: "emptyStock"
    };
    exports2.Prisma.VariantScalarFieldEnum = {
      id: "id",
      productId: "productId",
      name: "name",
      price: "price",
    ...
    exports2.Prisma.SaleScalarFieldEnum = {
      id: "id",
      storeId: "storeId",
    ...
    exports2.Prisma.CylinderLoanScalarFieldEnum = {
      id: "id",
      storeId: "storeId",
      saleId: "saleId",
    ...
    var config2 = {
      "generator": {
        ...
        "output": {
          "value": "E:\\backbone\\Projects\\swift_pos\\node_modules\\.prisma\\saas-client",
          "fromEnvVar": null
        },
        "config": {
          "engineType": "library"
    ...
    if (!fs2.existsSync(path.join(__dirname, "schema.prisma"))) {
      const alternativePaths = [
        "node_modules/.prisma/saas-client",
        ".prisma/saas-client"
      ];
...
var saasPrisma = globalForPrisma.saasPrisma ?? new import_saas_client.PrismaClient({
  log: ["error", "warn"]
});
...
// server/saas/config/tiers.ts
var TIER_ORDER = ["tindahan", "negosyo", "kumpanya"];
var SETUP_FEE_CENTAVOS = Number(process.env.SETUP_FEE ?? 149900);
...
var TIERS = {
  tindahan: {
    ...
    priceMonthlyCentavos: Number(process.env.TINDAHAN_PRICE ?? 49900),
  ...
  negosyo: {
    ...
    priceMonthlyCentavos: Number(process.env.NEGOSYO_PRICE ?? 99900),
  ...
  kumpanya: {
    ...
    priceMonthlyCentavos: Number(process.env.KUMPANYA_PRICE ?? 199900),
    maxBranches: null,
    maxUsers: null,
    features: [
...
function tierRank(tier) {
  const id2 = (tier ?? "").toLowerCase();
  const idx = TIER_ORDER.indexOf(id2);
  return idx >= 0 ? idx : -1;
}
...
function getTierFeatures(tier) {
  const id2 = normalizeLegacyPlan(tier).tier;
  const rank = tierRank(id2);
  return {
...
// server/saas/services/subscriptionService.ts
function subscriptionAllowsAccess(status, trialEnd) {
  const s = status.toLowerCase();
  if (s === "active") return { access: true };
  if (s === "trialing") {
    if (trialEnd && new Date(trialEnd) < /* @__PURE__ */ new Date()) {
...
async function ensureOrgSubscription(organizationId) {
  ...
  return saasPrisma.organizationSubscription.create({
    data: {
      ...
      trialEnd: status === "trialing" ? trialEnd ?? new Date(now.getTime() + TRIAL_DAYS * 864e5) : trialEnd,
...
async function expireTrialIfNeeded(organizationId) {
  ...
  if (!sub) return null;
  if (sub.status === "trialing" && sub.trialEnd && new Date(sub.trialEnd) < /* @__PURE__ */ new Date()) {
...
async function createTrialSubscription(organizationId) {
  const now = /* @__PURE__ */ new Date();
  const trialEnd = new Date(now.getTime() + TRIAL_DAYS * 864e5);
  return saasPrisma.organizationSubscription.create({
    data: {
      organizationId,
      tier: "tindahan",
...
async function requestPlan(organizationId, tier) {
...
async function activateSubscription(organizationId, tier, opts) {
  await ensureOrgSubscription(organizationId);
  const now = /* @__PURE__ */ new Date();
  const periodEnd = new Date(now.getTime() + 30 * 864e5);
  const billingDue = opts?.extendDays ? new Date(now.getTime() + opts.extendDays * 864e5) : periodEnd;
  await saasPrisma.$transaction([
    saasPrisma.organizationSubscription.update({
      where: { organizationId },
      data: {
        ...
        ...opts?.setupFeePaid !== void 0 ? { setupFeePaid: opts.setupFeePaid } : {}
      }
    }),
    saasPrisma.organization.update({
      where: { id: organizationId },
...
async function assertCanAddBranch(organizationId) {
  ...
  if (limits.maxBranches !== null && storeCount >= limits.maxBranches) {
    const upgradeTo = limits.tier === "tindahan" ? "negosyo" : limits.tier === "negosyo" ? "kumpanya" : null;
    return {
      ok: false,
...
function buildSubscriptionPublicPayload(org, sub, usage) {
...
// server/saas/middleware/suspendedCheck.ts
function isSubscriptionAllowlisted(path, method) {
...
async function suspendedCheckMiddleware(req, res, next) {
  const auth = req.auth;
  if (!auth) return next();
  if (auth.organizationId === null && auth.role === "super_admin") {
    return next();
  ...
  try {
    ...
    if (!access) {
      return res.status(403).json({
        message: reason === "trial_expired" ? "Trial expired. Please choose a plan to continue." : "Subscription inactive. Please choose a plan or contact support.",
        code: "SUBSCRIPTION_LOCKED",
...
// server/saas/middleware/superAdmin.ts
...
// server/saas/middleware/owner.ts
...
var mockProducts = [
  {
    ...
    variants: [
      ...
      { id: "v2", productId: "1", name: "Medium (500ml)", price: 2, stock: 40 },
      { id: "v3", productId: "1", name: "Large (1L)", price: 3, stock: 30 }
    ]
  },
  {
    id: "2",
  ...
  },
  {
    ...
    variants: [
      ...
      { id: "v8", productId: "5", name: "Cheese Burger", price: 6, stock: 20 },
  ...
  },
  {
    ...
    status: "active",
    variants: [
      { id: "v13", productId: "9", name: "Espresso", price: 2, stock: 50 },
      { id: "v14", productId: "9", name: "Cappuccino", price: 3, stock: 45 },
...
// server/saas/services/cylinderService.ts
async function processCylinderLinesOnSale(tx, ctx) {
  for (const line of ctx.lines) {
    ...
    const unitDeposit = ctx.collectDeposits && (product.depositAmount ?? 0) > 0 ? product.depositAmount : 0;
...
async function voidCylinderLoansForSale(tx, saleId) {
  ...
  await tx.cylinderLoan.updateMany({
    where: { saleId, status: "out" },
    data: { status: "written_off", returnedAt: /* @__PURE__ */ new Date() }
  });
}
async function returnCylinderLoan(storeId, loanId, options) {
...
async function listCylinderLoans(storeId, status = "out") {
...
async function getCylinderStats(storeId) {
  ...
  for (const p of tracked) {
    ...
    const empty2 = p.emptyStock ?? 0;
    const threshold = p.lowStockThreshold ?? 0;
    if (filled <= 0) outOfFilledCount += 1;
    else if (filled <= threshold) lowFilledCount += 1;
    if (empty2 <= 0) outOfEmptyCount += 1;
    else if (empty2 <= threshold) lowEmptyCount += 1;
  }
...
function isSameUtcDay(a, b2) {
  return a.getUTCFullYear() === b2.getUTCFullYear() && a.getUTCMonth() === b2.getUTCMonth() && a.getUTCDate() === b2.getUTCDate();
}
function assertCanVoidSale(sale, actor) {
  if (actor.role === "owner") return;
  if (actor.role === "cashier") {
    if (sale.cashierId !== actor.userId) {
      throw new Error("Forbidden: cashiers can only void their own sales");
    }
    if (!isSameUtcDay(sale.createdAt, /* @__PURE__ */ new Date())) {
...
async function voidSale(id2, storeId, actor) {
  ...
  const voidedAt = /* @__PURE__ */ new Date();
  return saasPrisma.$transaction(async (tx) => {
    ...
    await voidCylinderLoansForSale(tx, id2);
...
var productTemplates = mockProducts.map((p, idx) => {
  ...
  return {
    ...
    marginPercentage: p.marginPercentage ?? 25 + idx % 15,
...
// server/saas/routes/admin.ts
var PAID_PLAN_FILTERS = ["tindahan", "negosyo", "kumpanya", "suspended"];
var LEGACY_NON_BILLING_PLANS = [
  "free",
  "Free",
  "suspended",
  "Suspended"
...
router.get("/organizations", async (req, res) => {
  try {
    ...
    const orgs = await saasPrisma.organization.findMany({
      where: where.length > 0 ? { AND: where } : void 0,
      orderBy: { createdAt: "desc" },
      include: {
        _count: { select: { stores: true, users: true } }
      }
...
router.get("/organizations/:id", async (req, res) => {
  try {
    ...
    res.json({
      ...
      tierLimits: TIERS,
      users: users.map((u) => {
        const { password: _p2, storeAccess, ...uRest } = u;
...
router.patch("/organizations/:id", async (req, res) => {
  try {
    ...
    } else if (plan !== void 0 && isTierId(plan)) {
    ...
    if (setupFeePaid !== void 0 && !activateTier && plan === void 0) {
      await saasPrisma.organizationSubscription.update({
        where: { organizationId: req.params.id },
        data: { setupFeePaid }
      });
    ...
    if (billingDueDate !== void 0)
      updateData.billingDueDate = billingDueDate && billingDueDate.trim() ? new Date(billingDueDate) : null;
    if (phone !== void 0) updateData.phone = phone?.trim() || null;
    if (email !== void 0) updateData.email = email?.trim() || null;
    if (address !== void 0) {
...
router.delete("/api/admin/organizations/:id", async (req, res) => {
  try {
...
router.post("/api/admin/organizations/:id/users", async (req, res) => {
  try {
    ...
    const { password: _p2, ...sanitized } = user;
...
router.patch("/api/admin/organizations/:orgId/users/:userId", async (req, res) => {
  try {
    ...
    const updateData = {};
    if (name !== void 0 && name.trim()) updateData.name = name.trim();
    ...
    const { password: _p2, ...sanitized } = updated;
    res.json(sanitized);
  } catch (error) {
...
router.delete("/api/admin/organizations/:orgId/users/:userId", async (req, res) => {
  try {
    const { orgId, userId } = req.params;
    const user = await saasPrisma.user.findFirst({
      where: { id: userId, organizationId: orgId }
...
router.delete("/organizations/:orgId/stores/:storeId", async (req, res) => {
  try {
    const { orgId, storeId } = req.params;
    const existing = await saasPrisma.store.findFirst({
      where: { id: storeId, organizationId: orgId }
    ...
    res.status(400).json({ message: error.message ?? "Failed to delete store" });
  }
});
router.post("/api/admin/organizations/:orgId/notifications", async (req, res) => {
  try {
    const orgId = req.params.orgId;
    ...
    const msg = error instanceof Error ? error.message : typeof error === "object" && error !== null && "message" in error ? String(error.message) : "Failed to create notification";
...
router.post("/seed-demo", async (_req, res) => {
  try {
    ...
    const msg = error instanceof Error ? error.message : typeof error === "object" && error !== null && "message" in error ? String(error.message) : "Failed to seed demo data";
...
// server/saas/services/categoryService.ts
...
async function updateCategory(id2, storeId, name) {
  const existing = await saasPrisma.category.findFirst({ where: { id: id2, storeId } });
...
async function deleteCategory(id2, storeId) {
  const existing = await saasPrisma.category.findFirst({ where: { id: id2, storeId } });
  if (!existing) throw new Error("Category not found");
...
// server/saas/services/productService.ts
...
async function getProductById(id2, storeId) {
...
async function updateProduct(id2, storeId, input) {
  const existing = await saasPrisma.product.findFirst({ where: { id: id2, storeId } });
  if (!existing) throw new Error("Product not found");
  const updateData = {};
  for (const [key, val] of Object.entries(input)) {
    if (val !== void 0 && key !== "barcode" && key !== "qrCode") {
      updateData[key] = val;
    }
  }
  const itemCodeToUse = input.itemCode ?? existing.itemCode;
  if (itemCodeToUse) {
    const shouldGenerate = input.itemCode !== void 0 || !existing.barcode || !existing.qrCode;
    if (shouldGenerate) {
      try {
...
async function deleteProduct(id2, storeId) {
  const existing = await saasPrisma.product.findFirst({ where: { id: id2, storeId } });
  if (!existing) throw new Error("Product not found");
  await saasPrisma.variant.deleteMany({ where: { productId: id2 } });
...
async function createVariant(productId, storeId, input) {
  const product = await saasPrisma.product.findFirst({
    where: { id: productId, storeId }
  });
  if (!product) throw new Error("Product not found");
...
async function updateVariant(id2, storeId, input) {
...
async function deleteVariant(id2, storeId) {
  const variant = await saasPrisma.variant.findUnique({
    where: { id: id2 },
    include: { product: true }
  });
  ...
  return saasPrisma.$transaction(async (tx) => {
    await tx.variant.delete({ where: { id: id2 } });
...
async function getUsersByStore(storeId) {
  ...
  if (!store) throw new Error("Store not found");
  return store.userAccess.map((ua2) => ua2.user);
}
...
async function createUser(organizationId, storeIds, input) {
  const existing = await saasPrisma.user.findUnique({
...
async function updateUser(id2, organizationId, input) {
  ...
  const user = await saasPrisma.$transaction(async (tx) => {
    ...
    if (storeIds !== void 0) {
      await tx.userStore.deleteMany({ where: { userId: id2 } });
      const org = await tx.organization.findUnique({
        where: { id: organizationId },
        include: { stores: { select: { id: true } } }
      });
      const validIds = (storeIds || []).filter(
        (sid) => org?.stores.some((s) => s.id === sid)
      );
      const toAssign = validIds.length > 0 ? validIds : org?.stores.map((s) => s.id) ?? [];
      for (const storeId of toAssign) {
        await tx.userStore.create({
          data: { userId: id2, storeId }
        });
      }
    }
    return updated;
...
async function deleteUser(id2, organizationId) {
  const existing = await saasPrisma.user.findFirst({
    where: { id: id2, organizationId }
  });
  if (!existing) throw new Error("User not found");
  await saasPrisma.userStore.deleteMany({ where: { userId: id2 } });
  return saasPrisma.user.delete({ where: { id: id2 } });
}
...
var DEMO_CREDENTIALS = {
  ...
  owner: { email: "owner@demo.com", password: DEFAULT_PASSWORD2, role: "owner" },
  cashier: { email: "cashier@demo.com", password: DEFAULT_PASSWORD2, role: "cashier" }
};
async function runBootstrapSeed() {
  ...
  console.log(`[Bootstrap] Created default accounts (password: ${DEFAULT_PASSWORD2}):`);
...
var CONTROL_CHARS = /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/;
var UNSAFE_DISPLAY_CHARS = /[<>;"\\]/;
var UNSAFE_DISPLAY_PATTERNS = /--|<script|javascript:/i;
function validateDisplayName(value, fieldLabel, maxLen = 64) {
...
app.get("/api/org/users", authMiddleware, suspendedCheckMiddleware, async (req, res) => {
  try {
    ...
    const sanitized = users.map((u) => {
      const { password: _p2, storeAccess, ...rest } = u;
...
app.post("/api/auth/signup", signupLimiter, async (req, res) => {
  try {
    ...
    const { password: _p2, ...sanitizedUser } = user;
    res.status(201).json({
      token,
      user: sanitizedUser,
      organization: org,
...
ownerRouter.put("/api/categories/:id", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const { name } = req.body;
...
ownerRouter.delete("/api/categories/:id", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    await deleteCategory(req.params.id, storeId);
    res.status(204).send();
  } catch (error) {
    console.error(error);
...
ownerRouter.get("/api/users", async (req, res) => {
  try {
    const storeId = req.storeId;
...
ownerRouter.post("/api/users", async (req, res) => {
  try {
    const storeId = req.storeId;
    const organizationId = req.organizationId;
    if (!storeId || !organizationId) return res.status(400).json({ message: "storeId is required" });
    ...
    const storesToAssign = validStoreIds.length > 0 ? validStoreIds : org?.stores.map((s) => s.id) ?? [storeId];
...
ownerRouter.put("/api/users/:id", async (req, res) => {
  try {
    const organizationId = req.organizationId;
    if (!organizationId) return res.status(400).json({ message: "Not authorized" });
    const { name, email, password, role, storeIds } = req.body;
    if (role !== void 0 && role !== "owner" && role !== "cashier") {
      return res.status(400).json({ message: "Role must be owner or cashier" });
    }
...
ownerRouter.delete("/api/users/:id", async (req, res) => {
  try {
    const organizationId = req.organizationId;
    if (!organizationId) return res.status(400).json({ message: "Not authorized" });
    await deleteUser(req.params.id, organizationId);
    res.status(204).send();
  } catch (error) {
    console.error(error);
    res.status(400).json({ message: error.message ?? "Failed to delete user" });
...
ownerRouter.post("/api/menu-categories", async (req, res) => {
  try {
    ...
    const row = await createMenuCategory(storeId, safeName);
    res.status(201).json(row);
  } catch (error) {
    console.error(error);
    res.status(fnbErrorStatus(error)).json({ message: error.message ?? "Failed" });
...
ownerRouter.patch("/api/menu-categories/:id", async (req, res) => {
  try {
    ...
    const { name } = req.body;
    const safeName = validateDisplayName(name, "Menu category name");
    const row = await updateMenuCategory(req.params.id, storeId, safeName);
    res.json(row);
  } catch (error) {
    console.error(error);
    res.status(fnbErrorStatus(error)).json({ message: error.message ?? "Failed" });
...
protectedRouter.get("/api/products/:id", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
...
protectedRouter.get("/api/products/:productId/variants", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const variants = await listVariantsByProduct(req.params.productId, storeId);
    res.json(variants);
  } catch (error) {
    console.error(error);
...
protectedRouter.get("/api/sales/void-count", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const { from, to: to2 } = req.query;
    const count = await countVoidedSales(storeId, {
      from: from ? new Date(from) : void 0,
      to: to2 ? new Date(to2) : void 0
    });
    res.json({ count });
...
protectedRouter.get("/api/sales/:id", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const sale = await getSaleById(req.params.id, storeId);
...
protectedRouter.post("/api/sales/:id/void", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const auth = req.auth;
    ...
    const sale = await voidSale(req.params.id, storeId, actor);
    if (!sale) return res.status(404).json({ message: "Sale not found" });
    res.json(sale);
  } catch (error) {
...
protectedRouter.post("/api/sales", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const body = req.body;
    ...
    const rawPaymentMethod = body.paymentMethod?.toLowerCase();
    ...
    const sale = await createSale({
      ...
      gcashTransactionId: body.gcashTransactionId,
      customerName: body.customerName,
      customerPhone: body.customerPhone,
      collectDeposits: body.collectDeposits
    });
...
protectedRouter.get("/api/cylinder-loans", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const status = req.query.status || "out";
    const valid = ["out", "returned", "written_off", "all"];
    const loans = await listCylinderLoans(
      storeId,
      valid.includes(status) ? status : "out"
    );
    res.json(loans);
  } catch (error) {
    console.error(error);
...
protectedRouter.get("/api/cylinder-stats", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const stats = await getCylinderStats(storeId);
    res.json(stats);
  } catch (error) {
    console.error(error);
...
protectedRouter.post("/api/cylinder-loans/:id/return", async (req, res) => {
  try {
    const storeId = req.storeId;
    if (!storeId) return res.status(400).json({ message: "storeId is required" });
    const body = req.body;
    const loan = await returnCylinderLoan(storeId, req.params.id, {
...
orgRouter.get("/api/org/subscription", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
    await expireTrialIfNeeded(orgId);
    ...
    res.json({
      ...
      setupFeeCentavos: Number(process.env.SETUP_FEE ?? 149900)
    });
  } catch (error) {
    console.error(error);
...
orgRouter.post("/api/org/subscription/request", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
...
orgRouter.post("/api/org/subscription/cancel-request", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
    if (req.auth?.role !== "owner") {
...
orgRouter.get("/api/stores", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
...
orgRouter.patch("/api/org", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
    if (req.auth?.role !== "owner") return res.status(403).json({ message: "Owner access required" });
...
orgRouter.get("/api/org/stores", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
    if (req.auth?.role !== "owner") return res.status(403).json({ message: "Owner access required" });
...
orgRouter.post("/api/org/stores", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
    if (req.auth?.role !== "owner") return res.status(403).json({ message: "Owner access required" });
...
orgRouter.patch("/api/org/stores/:id", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
    if (req.auth?.role !== "owner") return res.status(403).json({ message: "Owner access required" });
...
orgRouter.delete("/api/org/stores/:id", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) return res.status(400).json({ message: "No organization" });
    if (req.auth?.role !== "owner") return res.status(403).json({ message: "Owner access required" });
...
orgRouter.get("/api/notifications", async (req, res) => {
  try {
    const orgId = req.auth?.organizationId;
    if (!orgId) {
      return res.json([]);
