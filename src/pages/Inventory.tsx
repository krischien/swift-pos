...
const Inventory = () => {
  ...
  const outOfStockCount = products.filter((p) => hasZeroStock(p)).length;
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((p, idx, arr) => (
                      <span key={p} className="flex items-center gap-1">
                        ...
                        <Button variant={currentPage === p ? "default" : "outline"} size="sm" className="min-w-8 h-8 p-0" onClick={() => setCurrentPage(p)}>
                          {p}
                        </Button>
                      </span>
                    ))}
